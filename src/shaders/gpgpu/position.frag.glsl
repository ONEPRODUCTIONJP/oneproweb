...
// --- Ashima classic 3D simplex noise (compact form) ---
...
void main() {
  ...
  vec3 target = texture2D(targetMark, uv).xyz;
  vec3 toTarget = target - pos;
  vec3 accel = toTarget * uSpring;

  float flowTime = uTime * 0.12;
  vec3 flow = curlNoise(pos * 0.4 + flowTime) * uFlowStrength;
  accel += flow;
  ...
  if (uPointerActive > 0.5) {
    ...
    accel += normalize(toMouse + 1e-4) * uMouseStrength * falloff;
  }

  vec3 newPos = pos + accel * uDelta * 4.0;
  newPos = mix(newPos, pos, uDamping);
  newPos += toTarget * (1.0 - uDamping) * uSpring * uDelta * 6.0;
